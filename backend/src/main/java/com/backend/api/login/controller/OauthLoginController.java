...
public class OauthLoginController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "#### 성공"), @ApiResponse(responseCode = "에러", description = "#### 에러 이유를 확인 하십시오", content =@Content(schema = @Schema(implementation = ErrorResponse.class), examples = {@ExampleObject( name = "401_Auth-006", value = "Authorization Header가 없습니다."), @ExampleObject( name = "500", value = "소셜에서 받은 Token값이 아닙니다."),@ExampleObject(name="400_User-002", value ="이미 등록된 회원입니다."),@ExampleObject(name="400_User-001", value ="잘못된 로그인 타입 입니다. KAKAO 또는 NAVER로 작성해주세요.")}))})
    @PostMapping("/login")
